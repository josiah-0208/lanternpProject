...
<style type="text/css">@import url("/lPj/css/festMain.css?2");</style>
...
<div class="container_main">
	...
	<div class="filter_box">
		<div class="filter">
			<img src="/lPj/images/icons/filter.png" style="width: 25px">
		</div>				
	</div>
					...
					<c:if test="${today >= festival.start_date && festival.end_date >= today }">
						<div class="item" onclick="location.href='festView.do?fno=${festival.fno}'">
							<div class="item_thumb">
								<img class="fest_img" src="/lPj/images/festival/${festival.thumbnail }">
							</div>
			...
			<c:if test="${not empty loc }">
				<c:forEach var="festival" items="${listLoc }">
					<c:if test="${festival.start_date <= today && festival.end_date >= today }">
						<div class="item" onclick="location.href='festView.do?fno=${festival.fno}'">
							<img style="background: url(/lPj/images/festival/${festival.thumbnail}) no-repeat center; background-size: contain;">
							<div class="item_fname">${festival.fname }
							</div>
			...
			<div class="pre_btn">
				<c:if test="${startPage > PAGE_PER_BLOCK}">
					<button class="pre1" onclick="location.href='festMain.so?tab=${tab }&pageNum=${startPage - 1}&loc=${loc }'">
						<img alt="이전" src="/lPj/images/icons/arrow_left1.png">
						<img alt="이전" src="/lPj/images/icons/arrow_left1.png">
					</button> 
				</c:if>
				<c:if test="${pageNum > 1}">
			...
			<div class="next_btn">
				<c:if test="${currentPage < totalPage}">
					<button class="next1" onclick="location.href='festMain.so?tab=${tab }&pageNum=${currentPage + 1}&loc=${loc }'">
						<img alt="다음" src="/lPj/images/icons/arrow_right.png">
					</button>
				</c:if>
